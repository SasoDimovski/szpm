...
@section('content')
                ...
                <div class="hr-top" >
                    <div class="hr-top-label" >{{__('auth.forgotten')}}</div>
                </div>
                <label for="tab-1" class="tab">{{config('app.TITLE')}}</label>
                <form action="{{ url('/forgotten-post') }}" method="post">
                    @csrf
                    <input type="hidden" id="hash" name="hash" value="{{$hash}}">
                    <div class="group">
                        <label for="new-password" class="label">{{__('auth.forgotten.new_password')}}</label>
                        <input type="password" class="input" id="new-password" name="new-password" maxlength="30" placeholder="" >
                    </div>
                    <div class="group">
                        <label for="confirm-password" class="label">{{__('auth.forgotten.confirm_new_password')}}</label>
                        ...
                        <div class="label" style="width: 100%;text-align: right;" id="toggle_img"><img
                                ...
                                onclick="toggleSwap()"
                                title="{{__('auth.forgotten.toggle')}}"
                            ></div>
                    </div>
                    <div class="group">
                        <input type="submit" class="button" value="{{__('auth.forgotten.enter')}}">
                    </div>

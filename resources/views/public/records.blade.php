...
@if(count($records) > 0)
     ...
     @if($module=="Новости")
        ...
        @endforeach
        @elseif($module=="Весници" || $module == "Коха" || $module == "Билтени" || $module == "Пензионерски видици" || $module == "Книги")
                                <article class="post post-medium">
                                    <div class="row mb-3">
                @foreach($records as $record)
                            <div class="col-lg-3">
                                <div class="post-content">
                                    <div class="post-image slika_vest">
                                        <a href="{{ url('record/'.$record->id_module.'/'.$record->id_association.'/'.$record->id.'/'.$record->slug) }}">
                                            ...
                                            @if($record->picture)
                                                @if ($record->created_at < \Carbon\Carbon::create(2025, 10, 10))
                                                    <a href="{{ asset('vesnik/'. $record->lastDocument->file) }}" target="_blank">
                                                        <img src="{{ asset('vesnik/'. $record->picture) }}"  class="img-fluid img-thumbnail img-thumbnail-no-borders rounded-10" alt="{{ $record->title }}" /></a>
                                                @else
                                                    <img src="{{asset($path_upload . $record->id . '/' .  $record->picture)}}" class="img-fluid img-thumbnail img-thumbnail-no-borders rounded-10" alt="{{ $record->title }}" />
                                                @endif
                                            @endif
                                    ...
                                    <p>{{ $record->subtitle }}</p>
                                    <h2 class="font-weight-semibold pt-4 pt-lg-0 text-4 line-height-2 mb-2"><a href="{{ asset('vesnik/'. $record->lastDocument->file) }}" target="_blank">{{ $record->title }}
                                        </a></h2>
                                    <p class="mb-0">{{ $record->intro }}
                                    </p>
                                </div>
                            </div>
                @endforeach
